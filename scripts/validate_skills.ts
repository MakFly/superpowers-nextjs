...
/**
 * Validates the structure of superpowers-nextjs plugin
...
function validateSkills(): void {
  ...
  for (const entry of entries) {
    ...
    } else if (!frontmatter.name.startsWith('nextjs:')) {
      errors.push({
        file: skillFile,
        message: `Name must start with "nextjs:", got "${frontmatter.name}"`,
      });
    }
...
function validatePluginJson(): void {
  ...
  try {
    ...
    if (json.name && json.name !== 'superpowers-nextjs') {
      warnings.push({
        file: PLUGIN_JSON,
        message: `Plugin name should be "superpowers-nextjs", got "${json.name}"`,
      });
    }
...
// Main execution
console.log('\n===========================================');
console.log('Validating superpowers-nextjs plugin');
console.log('===========================================\n');
